...
export function ListEditorWithTitle({
  title,
  onClose,
  value,
  bounds,
  ...
  return (
    <BaseEditorComponent bounds={bounds} zIndex={50}>
      ...
      <div className="w-full flex no-wrap mb-4">

        <div className="mr-4 w-4 flex items-center">
          &nbsp;
        </div>
        <div className="w-full mr-4 flex items-center">
          <label className="uppercase font-bold text-xs mr-2  text-gray-600 ">Titulo</label>
          <input value={currentValue.title} onChange={e => setTitle(e.target.value)} type="text" className="border w-full border-gray-300 outline-0 rounded-md p-1 text-sm" />
        </div>
