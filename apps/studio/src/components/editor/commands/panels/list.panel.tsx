...
import { generateColorFromString } from "@editor/utils/colors";
...
export const ListPanel = ({
  ...
  return (
    <Command.Group heading={title} className="mt-2 mb-4 text-gray-500">
      {
        items.map((it: any) => (
          <Item onSelect={onSelect} title={it.title} value={it.id} subtitle={it.subtitle} key={it.id} color={it.color || color}>
